        ...
        async with self.lock:
            try:
                ...
                # Content extraction actions
                    ...
                    # For complex dynamic pages, try to interact with the page first
                    try:
                        # Check if we're on a site that needs special handling
                        url_lower = current_url.lower()

                        # For movie sites like Fandango, try to extract movie listings with specialized methods
                        if any(site in url_lower for site in ["fandango", "movie", "cinema", "theatre", "theater"]):
                            logging.info("Detected movie/cinema page, using specialized extraction")

                            # Try to find and click on movie listing elements first
                            try:
                                # Wait for movie listing elements to load
                                await page.wait_for_selector('div[data-qa="movie-name"], h3.fd-movie__title, a.poster-thumb', timeout=3000)
                                ...
                                more_buttons = await page.query_selector_all('button:has-text("Show More"), button:has-text("Load More"), a:has-text("Show More")')
                                for button in more_buttons[:2]:  # Limit to first 2 buttons to avoid infinite clicking
                                    try:
                                        await button.click()
                                        await asyncio.sleep(0.5)  # Brief pause after clicking
                                    except Exception as click_err:
                                        logging.warning(f"Failed to click 'Show More' button: {click_err}")
                            except Exception as interact_err:
                                logging.warning(f"Failed to interact with movie page elements: {interact_err}")

                            # Extract movie-specific content
                            try:
                                # Get movie titles and details using selectors common on movie sites
                                movie_data = await page.evaluate('''() => {
                                    // Common movie listing selectors across different sites
                                    const movieElements = document.querySelectorAll('div[data-qa="movie-name"], h3.fd-movie__title, .poster-container, .movie-detail, .movie-name, .movie-title');

                                    // If specific selectors fail, try broader approach
                                    const allMovieData = [];

                                    if (movieElements && movieElements.length > 0) {
                                        // Try to extract from specific elements first
                                        movieElements.forEach(el => {
                                            const title = el.textContent.trim();

                                            // Look for parent container that might have more details
                                            const container = el.closest('.movie-container, .movie-item, .poster-container, [data-qa="movie-wrapper"]');

                                            let details = "";
                                            if (container) {
                                                // Try to find rating
                                                const rating = container.querySelector('[data-qa="movie-rating"], .movie-rating, .rating')?.textContent.trim();

                                                // Try to find runtime
                                                const runtime = container.querySelector('[data-qa="movie-runtime"], .movie-runtime, .runtime')?.textContent.trim();

                                                if (rating || runtime) {
                                                    details = `${rating || ""} ${runtime || ""}`.trim();
                                                }
                                            }
                                        ...
                                        // Fallback: look for patterns in the page that suggest movie listings
                                        const possibleMovieElements = document.querySelectorAll('h2, h3, h4, .title, .name, strong');

                                        possibleMovieElements.forEach(el => {
                                            // Check if this looks like a movie title
                                            const text = el.textContent.trim();
                                            if (text && text.length > 3 && text.length < 50) {
                                                // Look at siblings or parent for details
                                                let details = "";

                                                // Check next element for details
                                ...
                                if movie_data and len(movie_data) > 0:
                                    # Format the movie data nicely
                                    movie_text = "Movies currently playing:\n\n"
                                    for movie in movie_data:
                                        ...
                                        if title:
                                            movie_text += f"• {title}"
                                            if details:
                                                movie_text += f" - {details}"
                                            movie_text += "\n"

                                    # Create a proper extraction result
                                    movie_content = {
                                        "text": movie_text,
                                        "metadata": {
                                            "source": current_url,
                                            "extraction_method": "specialized_movie"
                                        }
                                    }

                                    # Format as JSON
                                    movie_json = json.dumps(movie_content, indent=2, ensure_ascii=False)
                                    msg = f"Extracted from page:\n{movie_json}\n"

                                    # Cache and return the successful extraction
                                    self.extracted_content_cache[cache_key] = msg
                                    return ToolResult(output=msg)
                            except Exception as movie_err:
                                logging.error(f"Movie-specific extraction failed: {movie_err}")
                                # Continue with standard extraction methods
                    except Exception as special_err:
                        logging.error(f"Special handling extraction failed: {special_err}")
                        # Continue with standard extraction

                    # Continue with standard content extraction
                    try:
                        ...
                        try:
                            import markdownify
                            # Use better options for markdownify to preserve more structure
                        ...
                        # First try to get structured content for more reliable extraction
                        # Get all text content directly from the page
                        try:
                            page_text = await page.evaluate('''() => {
                                ...
                                let mainText = "";
                                if (mainContent) {
                                    mainText = getVisibleText(mainContent);
                                }

                                return {
                                    headings: headings,
                                    lists: lists,
                                    mainText: mainText || getVisibleText(document.body),
                                    title: document.title
                                };
                            }''')

                            # Add this structured content to help extraction
                            if page_text:
                                content = f"""
# Page Title: {page_text.get('title', '')}

## Main Headings:
{' / '.join(page_text.get('headings', []))}

## Structured Content:
{page_text.get('mainText', '')}

## Full Content:
{content}
"""
                        except Exception as e:
                            logging.warning(f"Failed to get structured content: {e}")
...
"""
                        ...
                        # Increase max content length for better extraction of complex pages
                        max_content_length = min(100000, len(content))
                        ...
                        if (
                            ...
                            try:
                                ...
                                # If the text says information is not available but we're on a page that likely has it,
                                # try a fallback extraction method
                                if "not available" in extracted_content.get("text", "").lower():
                                    logging.info("First extraction reported no information, trying alternate extraction")

                                    # Try a simpler extraction with just general page content
                                    try:
                                        # Get all visible text
                                        visible_text = await page.evaluate('''() => {
                                            return Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, h6, p, li, td, th, a'))
                                                .filter(el => {
                                        ...
                                        if visible_text and len(visible_text) > 100:
                                            # We found significant text, use it as a fallback
                                            fallback_content = {
                                                "text": f"Page content extraction: {visible_text[:2000]}",
                                                "metadata": {
                                                    "source": current_url,
                                            ...
                                            fallback_json = json.dumps(
                                                fallback_content, indent=2, ensure_ascii=False
                                            )
                                            msg = f"Extracted from page:\n{fallback_json}\n"
                                    except Exception as e:
                                ...
                                # Add better debugging for extraction issues
                                logging.error(f"Error parsing extraction result: {str(e)}")
                                logging.error(f"Raw response: {tool_call.function.arguments}")
                                msg = f"Error parsing extraction result: {str(e)}\nRaw response: {tool_call.function.arguments}"
                        else:
                            # Add more helpful information for debugging
                            logging.warning(f"No content extracted for goal: {goal}")
                            if response:
                                logging.warning(f"Response without tool calls: {response}")
                            msg = "No content was extracted from the page."

                            # Try alternate extraction if primary failed
                            try:
                                # Get important text directly
                                page_title = await page.title()
                                direct_content = {
                                    "text": f"Page title: {page_title}\nNote: The automated extraction could not process this page fully. The page may require user interaction to display content.",
                                    "metadata": {
                                        "source": current_url,
                                ...
                                # Format direct content as JSON
                                direct_json = json.dumps(
                                    direct_content, indent=2, ensure_ascii=False
                                )
                                msg = f"Extracted from page:\n{direct_json}\n"
                                ...
                                logging.error(f"Direct extraction failed: {direct_error}")

                        return ToolResult(output=msg)
                    except Exception as e:
                        # Provide a more helpful error message with better debugging
                        error_msg = f"Failed to extract content: {str(e)}"
                        logging.error(error_msg)
                        try:
                            # Try to extract a small sample of the content for debugging
                            sample = content[:1000] if 'content' in locals() else "Content not available"
                            logging.error(f"Content sample: {sample}")

                            # Try one last desperate extraction method
                            try:
                                page_title = await page.title()
                                return ToolResult(
